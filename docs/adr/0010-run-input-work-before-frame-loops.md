...
The shared clock has two stages: input, then tick. Pointer, scroll, mutation, and throttle flushes run in the input stage. Frame-loop callbacks run in the tick stage. An input callback queued before frame dispatch begins is eligible for that frame and runs before every eligible tick callback. A callback first queued during either stage runs in the next frame. Additional work coalesces into an eligible callback that has not run yet; once it has run, new work waits for the next frame. Order within either stage is not part of the contract.
