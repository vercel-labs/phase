...
| ------------------------------------------------------------------------------------------ | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Manual tick stepping (advancing the clock by hand, mainly for tests)                       | No       | Tests use fake `requestAnimationFrame`. A public API that exists for tests violates the lean rule.                                                                     |
...
| `useTween` on the shared clock                                                             | No       | A tween is finite and self-completing and needs no cross-loop synchronization. Joining the shared clock would add cost for nothing; the rationale lives in its source. |
