...
- **Execution** ranks current cost. Listings put `↑ in a per-frame path` first when a recurring frame callback, observer callback, or move handler runs the matched line. A layout read near a one-shot rAF may have little impact, while the same read in a move handler can stall every frame. Severity cannot show this difference. In one canvas app, 181 of 182 `forced-reflow` candidates were incidental, but severity ranked every candidate as critical. Execution ranking uses local rAF callback cycles, intrinsic JSX move-handler bodies, and a nearby-code heuristic for other drivers. It also recognizes callbacks passed directly to phase frame APIs imported from `@usephase/core`, `@usephase/react`, or the legacy `phase` and `phase/react` specifiers when their local bindings have no detected shadowing. This ranking never hides findings. A line called indirectly from a loop remains incidental and appears below lines with visible per-frame ownership.
...
| Signal                           | Severity | Noise   | Detects                                                                                                                 | Fix pointer                                                                                                                |
| -------------------------------- | -------- | ------- | ----------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
...
| `background-animation`           | high     | noisy   | `setInterval`, or a `setTimeout` that reschedules itself, driving transform/opacity work                                | [timed-sequences.md](./timed-sequences.md#first-choose-who-owns-the-timeline)                                              |
...
| `tailwind-layout-transition`     | high     | normal  | Static Tailwind transition-[...] utility that names a layout property                                                   | [audit.md](#step-15-css-loading-and-architecture-pass)                                                                     |
| `when-visible-no-fallback`       | high     | noisy   | `WhenVisible`/`WhenIdle` without a fallback; verify whether mount changes in-flow size **(JSX)**                        | [rendering-recipes.md](./rendering-recipes.md#what-not-to-compose)                                                         |
...
| `manual-synced-ref`              | dedup    | precise | `useRef(v)` + unconditional `ref.current = v` (shorthand exists)                                                        | [use-synced-ref.md](./use-synced-ref.md#when-to-use)                                                                       |
| `manual-stable-callback`         | dedup    | precise | `useCallback` with empty deps calling through a ref **(JSX)**                                                           | [use-stable-callback.md](./use-stable-callback.md#when-to-use)                                                             |
