...
- Use `setFps()` to change speed mid-run: only the scheduling gate changes, so `elapsed`, `delta`, the frame count, and `FrameState` identity stay continuous and no early frame leaks through the new cap. `setFps(undefined)` removes the cap, the same meaning `fps: undefined` has at creation.
