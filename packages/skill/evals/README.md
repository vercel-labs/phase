...
7. **specialized-shared-observers.** A framework-agnostic owner shares IO and RO instances across dynamic targets, consumes entry-derived output, unregisters each target, and disconnects at owner teardown. The scan retains both raw-observer findings; the rubric requires contract and cleanup evidence before permitting no change.
8. **external-renderer-ownership.** A reactive parent owns reduced-motion fallback while its child delegates playback and gestures to an external renderer. The scan retains ignored-motion and layout-read findings while proving the indirect move-handler detector boundary; the rubric preserves ownership and follows synchronous helper chains manually.
9. **terminal-state-persistent-layers.** Persistent fallback and live layers use transition completion, a one-shot reliability timeout, and one guarded terminal frame-state update. The scan retains the state finding; the rubric distinguishes terminal reconciliation from repeated frame-derived state without replacing recovery layers with Presence or Swap.
