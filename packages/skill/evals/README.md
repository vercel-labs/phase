...
Seed scenarios for evaluating an agent using the phase skill. Contributor tooling only: this directory belongs to the private `@usephase/skill` package and sits outside `skills/phase/`, so skill installations and security audits do not include adversarial fixtures. It is excluded from lint and formatting because fixtures are frozen test data and the goldens pin `file:line` positions. Scanner noise tiers are calibrated against real codebases per the "Recalibrating the scanner" procedure in `../AGENTS.md`; outcomes are encoded as executable `noMatch` examples and detection fixes, not committed as per-repo scorecards.
...
7. **specialized-shared-observers.** One class shares one IO and one RO across a changing set of elements, stops watching removed elements, and disconnects both observers. The scan keeps both raw-observer findings. The rubric checks the elements watched, observer data used, and cleanup before allowing "no change".
8. **external-renderer-ownership.** A parent shows a static fallback when reduced motion is on. The child uses an external renderer for playback and gestures. The scan keeps the ignored-motion and layout-read findings but confirms that the pointer detector misses the helper call. The rubric keeps the renderer APIs and follows same-file helpers by hand.
9. **terminal-state-persistent-layers.** Fallback and live layers stay mounted for recovery. They use a transition event, one backup timeout, and one guarded state update from `onTick`. The scan keeps the state finding. The rubric accepts the one-time update, rejects state written every frame, and keeps both layers instead of using `Presence` or `Swap`.
10. **smil-lifecycle.** An SVG route indicator uses indefinite SMIL playback, a delayed imperative start, and a reduced-motion branch that still moves. The scan assigns SMIL ownership to one per-file signal. The rubric requires inert first paint, static reduced-motion output, root lifecycle control, timer cleanup, and bounded compatibility claims without a new public hook.
11. **baseline-gate.** A workspace starts with one accepted critical finding. The gate writes and auto-detects a baseline, passes unchanged, then copies in one planted critical finding and proves only that source location is listed when the scan fails.
12. **reporting-route-and-options.** A clean article route reaches a parent layout and an optional video scrubber. The scan gates prove the scopes differ; the rubric checks report grouping, fix order, plain language, and the choice between phase, browser behavior, and deletion.
13. **shared-shell-investigation.** A clean article route inherits a width-animated chat panel with no established safe fix. The scan proves the candidate lives in the shared shell; the rubric requires investigation at the shared owner, a representative trace, and a compact handoff with revision, base path, and source map instead of a route-local workaround or mini-runbook.
14. **stated-intent-conflict.** A future-compatibility comment calls a hook a no-op while it still installs browser work and has no current producer. The rubric requires both sides, `Needs decision`, and no forced phase primitive before the intended behavior is chosen.
15. **runtime-priority-and-reduced-motion.** A phase-only audit contains a recurring layout read and a separate CSS animation without reduced-motion handling. The rubric keeps the browser-runtime fix queue separate from Reduced-motion accessibility.
16. **nested-shell-lifecycle.** A clean route inherits a shared shell whose dynamically imported canvas mounts and keeps working inside a CSS-hidden panel. Separate scans prove the lazy boundary remains part of the component-path audit without making scanner targets transitive.
17. **companion-page-performance.** A broad Next.js page audit contains Phase browser work, a React subscription and barrel import, and a server data waterfall. The rubric keeps the three audit scopes separate under a neutral report title.
18. **companion-skills-unavailable.** A broad Next.js audit runs with only Phase available. The rubric requires a visible React and Next.js coverage gap plus an installation offer, without improvised companion findings.
19. **mixed-phase-page-performance.** A deep Phase audit also asks about page load and the full rendering lifecycle. The rubric keeps the named Phase scope authoritative and hands adjacent React and Next.js concerns off without running full companion audits.
...
Until the agent runner lands, changes to companion orchestration require two bounded manual behavioral runs: one with the React and Next.js companion capabilities available and one with them withheld. Record the exact skills exposed in each run. The deterministic scan gates still run in CI; behavioral checks remain non-gating.
